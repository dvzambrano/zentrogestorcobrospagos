...
ElementApp = function() {
    return {
            ...
            this.store = new Ext.data.GroupingStore({
                ...
                listeners: {
                    load: config.app_showmessageonstoreloadsuccessful ? loadStoreSuccessful : function(store, records) { 
                        alertNoRecords(records, bundle.getMsg('element.tab.label').toLowerCase());
                    },
                ...
                tbar: [{
                    ...
                    listeners: {
                        click: function(button, eventObject, hideApply, callback) {
                            ...
                            window['ElementApp'].showWindow(button.getEl(), hideApply, callback);
                            App.mask.hide();
                        }
                    }
                },{
        ...
        showWindow : function(animateTarget, hideApply, callback){
            window['ElementApp'].window = App.showWindow(bundle.getMsg('element.window.title'), 370, 230, window['ElementApp'].formPanel, 
                function(button){
                    if(!button){
                        button = new Object;
                    ...
                    window['ElementApp'].formPanel.getForm().submit({
                        ...
                        success: function(form, action) {
                            ...
                            submitFormSuccessful('ElementApp', form, action, button, !records[0], function(){
